...
- The constant classes of `src/Model/` (and `Env`, `Asset`) mirror the Node.js SDK's `src/constants.ts`: same names, same keys, same values, same order — a set added on one side is added on the other. Never a string literal for an enumerated value in the documentation or the tests: always the constant (`Locale::FR`, `CoreType::STAKING`, `OrganizationCategory::B2B`); raw strings stay in the response fixtures and the expected request bodies only.
